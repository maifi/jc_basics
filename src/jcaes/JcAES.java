...
public class JcAES extends Applet {
	...
	short rsa_block_size = 128;
	short key_size_bits = 1024;
	...
	private JcAES(){
		//allocate all memory
		dataToEncrypt = new byte[rsa_block_size];
		dataToDecrypt = new byte[rsa_block_size];
		
		rsaEncrypted = new byte[rsa_block_size];
		rsaDecrypted = new byte[rsa_block_size];
		
		result = new byte[rsa_block_size];

		//KeyAgreement dhKeyAgreement = KeyAgreement.getInstance(KeyAgreement.ALG_EC_SVDP_DH, false);
		_rsaKeyPair = new KeyPair(KeyPair.ALG_RSA, (short) key_size_bits);
...
//				break;
			...
			case RSA_DECRYPT:
				...
				number_bytes = rsaCipher.doFinal(dataToDecrypt, (byte)0, (short)rsa_block_size, rsaDecrypted, (byte)0);
			...
			case EXPORT_RSA_EXP:
				try{
					...
					apdu.setOutgoing();
					apdu.setOutgoingLength((short)rsa_block_size);
					apdu.sendBytesLong(cmd, (short)0, (short)rsa_block_size);
				}catch(Exception e){
			...
			case EXPORT_RSA_MOD:
				try{
					...
					apdu.setOutgoing();
					apdu.setOutgoingLength((short)rsa_block_size);
					apdu.sendBytesLong(reply, (short)0, (short)rsa_block_size);
				}catch(Exception e){
					ISOException.throwIt(ISO7816.SW_DATA_INVALID);
				}
				break;
